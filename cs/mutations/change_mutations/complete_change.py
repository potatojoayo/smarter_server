...
class CompleteChange(graphene.Mutation):
    class Arguments:
    ...
    def mutate(cls, _, __, **kwargs):
        try:
            ...
            if is_normal is False:
                return CompleteChange(success=False, message=message)
            ...
            for input_change_detail in input_change_details:
                ...
                if change_gap != 0:
                    ...
                    old_order_detail.price_total += (old_order_detail.product_master.price_gym + price_work) * new_order_detail_quantity
                ...
                change_detail.save()
                change_order_detail = OrderDetail.objects.get(pk=change_detail.order_detail.id)
                change_order_detail.quantity = input_change_detail.changing_quantity
                change_order_detail.state = "교환완료"
            ...
            if is_change_price_exempt and total_changing_price > 0:
                SmarterPaidHistory.objects.create(change_request=change_request, amount=total_changing_price, reason="교환금액 지불")
            ...
            if is_delivery_price_exempt and delivery_price > 0:
                SmarterPaidHistory.objects.create(change_request=change_request, amount=delivery_price, reason="교환배송금액 지불")
            # change_request.delivery_price = delivery_price
            payment_amount +=  0 if is_delivery_price_exempt else delivery_price
            if payment_amount != 0:
                order_master = change_request.cs_request.order_master if change_request.cs_request else change_request.change_details.all().first().order_detail.order_master
                user = change_request.cs_request.gym.user if change_request.cs_request else change_request.user
                SmarterMoneyHistory.objects.create(
                    order_master=order_master,
                    history_number='R{}{}'.format(user.phone, datetime.now().strftime('%y%m%d%H%M%S')),
                    wallet=user.wallet,
                    transaction_type='차감' if payment_amount > 0 else '적립',
                    amount=payment_amount,
                    description='교환차감' if payment_amount > 0 else '교환적립'
                )
            user_wallet = change_request.cs_request.gym.user.wallet if change_request.cs_request else change_request.user.wallet
            ...
            change_request.save()
            user_wallet.save()
            return CompleteChange(success=True, message="교환이 성공적으로 완료되었습니다.")
        except Exception as e:
            print(e)
            import logging
            logger = logging.getLogger('myLog')
            logger.info(e)
            return CompleteChange(success=False, message="알 수 없는 에러가 발생했습니다. 개발팀 문의 바랍니다.")
